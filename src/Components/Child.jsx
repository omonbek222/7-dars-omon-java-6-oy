...
const HeroSection = () => {
  ...
  useEffect(() => {
    fetch("https://jsonplaceholder.typicode.com/users")
      ...
      .then((data) => setUsers(data.slice(0, 6))) // Dastlabki 6 foydalanuvchini olish
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-12 text-white">
        <div className="grid grid-cols-3 gap-4 max-w-4xl w-full">
          {users.map((user, index) => (
            <div key={user.id} className={`p-6 border ${index === 0 ? 'bg-blue-900 text-white' : 'bg-white text-blue-900'} rounded-lg shadow-md`}>
